...
class Material(SimpleObject):
    ...
    def _getFileList(self, dbfile, file_type):
        ...
        try:
            sql_select = """SELECT uuid FROM file WHERE mat_id=? AND file_type=? ORDER BY id DESC;"""
            
            # Execute the query.
            sop_files = super(Material, self).fetchAllSQL(dbfile, sql_select, [self._uuid, file_type])
...
class File(SimpleObject):
    ...
    def _initInstanceByUuid(self, uuid, dbfile):
        ...
        if not entry == None:
            ...
            self._uuid = str(uuid)
            self._material = str(entry[1])
            self._created_date = str(entry[2])
            self._modified_date = str(entry[3])
            self._filename = str(entry[4])
            self._file_type = str(entry[5])
            self._public = str(entry[6])
            self._alias = str(entry[7])
            self._status = str(entry[8])
            self._lock = str(entry[9])
            self._source = str(entry[10])
            self._operation = str(entry[11])
            self._operating_application = str(entry[12])
            self._caption = str(entry[13])
            self._description = str(entry[14])
        else:
            return(None)
